...
```
vault policy write terraformone - <<EOF
path "*" {
...
```bash
vault write auth/approle/role/terraformroleone \
    secret_id_ttl=10m \
    token_num_uses=10 \
    token_ttl=20m \
    token_max_ttl=30m \
    secret_id_num_uses=40 \
    token_policies=terraformone
```
...
```bash
vault read auth/approle/role/terraformrole/role-id
```
...
```bash
vault write -f auth/approle/role/terraformrole/secret-id
   ```
